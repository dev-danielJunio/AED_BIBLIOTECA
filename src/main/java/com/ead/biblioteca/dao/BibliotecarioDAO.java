...
public class BibliotecarioDAO {
    ...
    public void inserirBibliotecaria(Bibliotecario bibliotecaria) {
        String sql = "INSERT INTO biblioteca.tab_bibliotecarias (matricula, nome) VALUES (?, ?)";

        try (Connection conn = conexaoBanco.conectar();
             PreparedStatement stmt = conn.prepareStatement(sql)) {

            stmt.setInt(1, bibliotecaria.getMatricula());
            stmt.setString(2, bibliotecaria.getNome());

            stmt.executeUpdate();
        } catch (Exception e) {
            throw new RuntimeException("Erro ao inserir bibliotecária: " + e.getMessage());
        }
    }
}
